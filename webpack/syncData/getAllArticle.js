...
const getAllMarkdownFile = function(filePath){
  ...
  const markdownFile = walkFile(filePath).filter((p)=>{
   ...
   const result = markdownFile.map((file) =>{
      ...
      var subVal  = content.substring(3,subEnd)
      var subList = subVal.split("\n")
      var commentList = []
      subList.forEach(v => {
        var itemList = v.split(":")
        commentList[itemList[0]] = itemList[1]
      })
      var example = mDString.substring(subEnd+4,300)
      ...
      return {
        fileName: fileName,
        title:commentList["title"] || fileName,
        tag:commentList["tag"],
